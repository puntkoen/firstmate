...
bin/git-hooks/       one committed symlink per git hook name to bin/fm-attribution-guard.sh; fm-spawn arms them per task copy so a commit carrying agent attribution is refused, and that script owns the rules, the pass-throughs, and the residual gaps
